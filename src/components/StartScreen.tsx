  ...
  const FooterContent = () => (
    <div className="text-xs font-serif tracking-wider px-4 flex flex-col gap-3 w-full max-w-5xl">
      ...
      <div className="flex flex-col items-center gap-1 border-t border-white/10 pt-2 mt-1">
        ...
        >
          <span>如果你喜欢这个项目，欢迎在 原版 GitHub 仓库上点一颗 Star</span>
          <span className="text-yellow-500">★</span>
        </p>
        <p
          className="text-[10px] text-text-muted hover:text-accent-lantern transition-colors cursor-pointer pointer-events-auto scale-90 md:scale-100 origin-center"
          onClick={() => window.open('https://github.com/Plant-Paramour/pale-notes-LLMfree', '_blank')}
        >
          此为支持 OpenAI API 的 Fork 分支，访问本仓库
        </p>
      </div>
  ...
  const renderIntro = () => {
    ...
    <div className="min-h-screen w-full relative bg-background text-text-primary flex flex-col">
      ...
      <div className="flex-grow w-full flex flex-col items-center p-6 md:p-8 pb-24 md:pb-48 relative z-10">
        ...
        <div className="md:hidden w-full mt-12 pb-8">
          <FooterContent />
        </div>
        </div>
      </div>
      ...
      {/* Footer Info & Stats - Desktop Fixed */}
      <div className="hidden md:flex w-full z-50 flex-col items-center pointer-events-none select-none bg-gradient-to-t from-black/95 via-black/80 to-transparent pb-6 pt-8 fixed bottom-0">
        <FooterContent />
      </div>
