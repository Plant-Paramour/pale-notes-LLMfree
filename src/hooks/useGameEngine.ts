...
export function useGameEngine(): GameEngineReturn {
  ...
  const apiModel = useUIStore(state => state.apiModel);
  ...
  const processTurn = async (actionText: string, actionType: 'option' | 'custom' = 'option', storyContext?: string, requiredOptions?: any[], goalOptions?: any[]) => {
    ...
    try {
      ...
      const narrativeReq: DeepSeekChatMessage[] = [
        systemMessage,
      ...
      updatedStore.addHistory({ role: 'assistant', content: fullContent, timestamp: Date.now() });
      ...
      setDebugDataInput(dataContext);

      const dataReq: DeepSeekChatMessage[] = [
